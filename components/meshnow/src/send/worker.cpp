...
void SendWorker::stop() {
    ...
    // clear waitbits
    waitbits_.clear(SEND_SUCCESS_BIT | SEND_FAILED_BIT);
}
...
void SendWorker::onSend(const uint8_t*, esp_now_send_status_t status) {
    ...
    waitbits_.set(status == ESP_NOW_SEND_SUCCESS ? SEND_SUCCESS_BIT : SEND_FAILED_BIT);
}
...
void SendWorker::runLoop(const std::stop_token& stoken) {
    while (!stoken.stop_requested()) {
        ...
        auto optional = send_queue_.popPacket(portMAX_DELAY);
        if (!optional) {
            ESP_LOGE(TAG, "Failed to popPacket from send queue");
            continue;
        }
        ...
        // wait for callback
        auto bits = waitbits_.wait(SEND_SUCCESS_BIT | SEND_FAILED_BIT, true, false, SEND_TIMEOUT);
