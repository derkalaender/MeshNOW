...
TickType_t HandShaker::nextActionAt() const noexcept {
    ...
    if (searching_for_parents_) {
        ...
        // next time until we need to send a should_connect request
...
void HandShaker::performAction() {
    ...
    if (searching_for_parents_) {
        ...
        // check that the last should_connect request timed out, otherwise return and keep waiting
        if (xTaskGetTickCount() - last_connect_request_time_ < CONNECT_TIMEOUT) return;

        // try to should_connect to the best parent
        tryConnect();
    }
}
...
void HandShaker::receivedConnectResponse(const MAC_ADDR& mac_addr, bool accept, std::optional<MAC_ADDR> root_mac_addr) {
    ...
    if (accept) {
        ...
        // set the tick time to 0 so that we try to should_connect to another parent
        last_connect_request_time_ = 0;
    }
}
...
void HandShaker::receivedConnectRequest(const MAC_ADDR& mac_addr) {
    ...
    // TODO check can accept
    {
        ...
        if (!feature.get().isOk()) {
            ESP_LOGI(TAG, "Failed to send should_connect reply. Ignoring.");
            return;
        }
    }
    ...
    if (!state_->isRoot()) {
        ...
        if (!feature.get().isOk()) {
            ESP_LOGI(TAG, "Failed to send child should_connect event. Not adding to router.");
            return;
        }
    }
