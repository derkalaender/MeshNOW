...
static int iperf_get_socket_error_code(int sockfd) { return errno; }
...
static esp_err_t iperf_start_report(void) {
    ...
    ret = xTaskCreatePinnedToCore(iperf_report_task, IPERF_REPORT_TASK_NAME, IPERF_REPORT_TASK_STACK, NULL,
...
static void socket_recv(int recv_socket, struct sockaddr_storage listen_addr, uint8_t type) {
    ...
    int actual_recv = 0;
    socklen_t socklen =
        (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6) ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in);
...
static void socket_send(int send_socket, struct sockaddr_storage dest_addr, uint8_t type, int bw_lim) {
    ...
    while (!s_iperf_ctrl.finish) {
        if (period_us > 0) {
            ...
            if (actual_send > 0) {
                // Last packet "send" was successful, check how much off the previous loop duration was to the ideal
                // send period. Result will adjust the next send delay.
                delay_us += period_us + (int32_t)(prev_time - send_time);
            } else {
                // Last packet "send" was not successful. Ideally we should try to catch up the whole previous loop
                // duration (e.g. prev_time - send_time). However, that's not possible since the most probable reason
                // why the send was unsuccessful is the HW was not able to process the packet. Hence, we cannot queue
                // more packets with shorter (or no) delay to catch up since we are already at the performance edge. The
                // best we can do is to reset the send delay (which is probably big negative number) and start all over
                // again.
                delay_us = 0;
            }
            prev_time = send_time;
        ...
        *pkt_id_p = htonl(pkt_cnt);  // datagrams need to be sequentially numbered
        ...
        // The send delay may be negative, it indicates we are trying to catch up and hence to not delay the loop at
        // all.
        if (delay_us > 0) {
...
static esp_err_t IRAM_ATTR iperf_run_tcp_server(void) {
    ...
    ESP_GOTO_ON_FALSE((s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6 || s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV4),
    ...
    if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6) {
        ...
        ESP_GOTO_ON_FALSE((err == 0), ESP_FAIL, exit, TAG, "Socket unable to bind: errno %d, IPPROTO: %d", errno,
                          AF_INET6);
...
esp_err_t iperf_start(iperf_cfg_t *cfg) {
    ...
    ret = xTaskCreatePinnedToCore(iperf_task_traffic, IPERF_TRAFFIC_TASK_NAME, IPERF_TRAFFIC_TASK_STACK, NULL,
                                  IPERF_TRAFFIC_TASK_PRIORITY, NULL, portNUM_PROCESSORS - 1);
    if (ret != pdPASS) {
