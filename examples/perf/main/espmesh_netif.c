...
static esp_err_t mesh_netif_transmit_from_root_ap(void *h, void *buffer, size_t len) {
    ...
    if (MAC_ADDR_EQUAL(dest_addr.addr, eth_broadcast)) {
        ...
        esp_mesh_get_routing_table((mesh_addr_t *)&s_route_table, CONFIG_MESH_ROUTE_TABLE_SIZE * 6, &route_table_size);
...
static esp_netif_t *create_mesh_link_ap(void) {
    ...
    esp_netif_config_t cfg = {.base = &base_cfg, .driver = NULL, .stack = ESP_NETIF_NETSTACK_DEFAULT_WIFI_AP};
